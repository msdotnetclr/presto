...
public class PageProcessor
{
    ...
    {
        WorkProcessor<Page> processor = process(session, yieldSignal, memoryContext, singlePageProcessor(memoryContext, page));
    ...
    {
        return pages.flatTransform(new PageFilterAndProject(session, yieldSignal, memoryContext));
    }

    private WorkProcessor<Page> singlePageProcessor(AggregatedMemoryContext memoryContext, Page page)
    {
        return WorkProcessor.create(new SingletonPage(memoryContext, page));
    }

    private static class SingletonPage
    ...
    private class PageFilterAndProject
            implements WorkProcessor.Transformation<Page, WorkProcessor<Page>>
    {
        ...
        private PageFilterAndProject(ConnectorSession session, DriverYieldSignal yieldSignal, AggregatedMemoryContext memoryContext)
        {
